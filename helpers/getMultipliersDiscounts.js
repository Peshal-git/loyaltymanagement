...
const getMultiplier = async (spendingType) => {
  const multiplierForSpending = await Pricing.findOne({
    "spendingMultiplier.spendingType": spendingType,
  });
...
const getDiscount = async (id) => {
  ...
  const discountPercent = await Pricing.findOne({
    "tierDiscount.tier": user.tier,
  });
