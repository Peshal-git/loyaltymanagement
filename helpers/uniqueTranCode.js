...
const generateUniqueCode = async () => {
  try {
    ...
    do {
      ...
      const tranCodeValuesInTransaction = tranCodesInTransaction.map(
        (item) => item.tranCode
      );
      const tranCodeValuesInRedeem = tranCodesInRedeem.map(
        (item) => item.tranCode
      );
      tranCodeExists =
        tranCodeValuesInTransaction.includes(generatedCode) ||
        tranCodeValuesInRedeem.includes(generatedCode);
